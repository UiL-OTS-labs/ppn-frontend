...
from .options import TypeCollectionOptions, ResourceOptions, \
...
class _CollectionBaseMetaclass(ResourceMetaclass):
    """
    Base class for the collection metaclasses. It extends the
    ResourceCollectionMetaclass, which contains most code we need. However,
    this class overrides _options to None in order to discourage programmers
    from using this class directly. (Otherwise it would act like a resource).

    It also implements 2 overrides: register_class and add_to_class. The
    first is overridden in order to register classes using this as
    collections. The second is changed to remove the Field support,
    as collection don't have fields. (And it creates import errors....)
    """
    _options = None
...
class ResourceCollectionMetaclass(_CollectionBaseMetaclass):
    """Metaclass for resource collections.
    """
